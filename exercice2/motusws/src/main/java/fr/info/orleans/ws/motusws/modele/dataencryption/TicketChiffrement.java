...
public class TicketChiffrement {
    ...
    public TicketChiffrement(String secret) {
        byte[] key;
        try {
     ...
     * @param ticketPartie : invitation à chiffrer
     * @return : token correspondant au ciffrement de l'invitation en paramètre
     */

    public String chiffrement(TicketPartie ticketPartie) {

        try {
            cipher.init(ENCRYPT_MODE,
                    secretKey);
            String strToEncrypt = this.objectMapper.writeValueAsString(ticketPartie);
    ...
    public TicketPartie dechiffrement(String token) throws TicketInvalideException {
        try {
            cipher.init(Cipher.DECRYPT_MODE, secretKey);
            String jsonString = new String(cipher.doFinal(Base64.getDecoder().decode(token)));
            return objectMapper.readerFor(TicketPartie.class).readValue(jsonString);
